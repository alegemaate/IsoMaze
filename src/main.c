...
void setup() {
  ...
  // Make a cube
  tile_types[0] = generate_cube (Transparent, TILE_WIDTH, TILE_LENGTH);
  tile_types[1] = generate_cube (Silver, TILE_WIDTH, TILE_LENGTH);
  ...
  for (int i = 2; i < NUM_COLORS; i++) {
    ...
    switch (random_num) {
      case 0:
        random_color = Turquoise;
        break;

      case 1:
        random_color = Emerald;
        break;

      case 2:
        random_color = BelizeHole;
        break;

      case 3:
        random_color = Amethyst;
        break;

      case 4:
        random_color = SunFlower;
        break;
      ...
      case 6:
      default:
        random_color = Pomegranate;
        break;
    }

    tile_types[i] = generate_cube (random_color, TILE_WIDTH, TILE_LENGTH);
  }

  // Player
  player = generate_cube (MidnightBlue, TILE_WIDTH, TILE_LENGTH);
}
...
void draw() {
  // Background
  rectfill (buffer, 0, 0, buffer -> w, buffer -> h, Clouds);
